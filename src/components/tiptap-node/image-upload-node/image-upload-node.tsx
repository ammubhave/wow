...
const DropZoneContent: React.FC<{maxSize: number; limit: number}> = ({maxSize}) => (
  <>
    ...
    <div className="tiptap-image-upload-content">
      ...
      <span className="tiptap-image-upload-subtext">Maximum {maxSize / 1024 / 1024}MB.</span>
    </div>
  </>
);
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        const imageNodes = urls.map((url, index) => {
          ...
          const kind = files[index]?.type.split("/")[0] || "image";
          if (kind === "audio") {
            return {type: "audio", attrs: {...extension.options, src: url}};
          }
          return {
            type: "image",
            attrs: {...extension.options, src: url, alt: filename, title: filename},
          };
        });
