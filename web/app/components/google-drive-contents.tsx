...
export function GoogleDriveCardContents({
  ...
  const folderMutation = trpc.workspaces.setGoogleFolderId.useMutation({
    onSuccess: () => {
      utils.invalidate();
    },
  });
  const fileMutation = trpc.workspaces.setGoogleTemplateFileId.useMutation({
    onSuccess: () => {
      utils.invalidate();
    },
  });
  ...
  const state = trpc.workspaces.getGoogleTokenState.useQuery(workspaceId!);
  ...
  return (
    <>
      <CardHeader className="px-7">
        ...
        <CardDescription>
          You can connect your Google Drive account to this workspace. This
          allows your workspace to automatically create new spreadsheets
          whenever you create a new puzzle.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {state.data ? (
                  ...
                  >
                    ...
                    <div className="pl-4 flex flex-1 items-center justify-between">
                      ...
                      <Button
                        ...
                        variant={
                          state.data.state === 1 ? "default" : "secondary"
                        }
                        onClick={handleOpenFolderPicker}
                      >
                        Select Folder
                    ...
                    <div className="pl-4 flex flex-1 flex-col sm:flex-row gap-2 justify-between">
                      ...
                      <div>
                        <Button
                          ...
                          variant={
                            state.data.state === 2 ? "default" : "secondary"
                          }
                          onClick={handleOpenFilePicker}
                        >
                          Select File
  ...
  return (
    <form method="GET" action="https://accounts.google.com/o/oauth2/v2/auth">
      <input
        type="hidden"
        name="client_id"
        value={import.meta.env.VITE_GOOGLE_API_CLIENT_ID}
      />
      <input
        type="hidden"
        name="redirect_uri"
